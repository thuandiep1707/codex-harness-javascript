...
9. Spawn Brain for final acceptance and close/verify the Brain child. If it returns `blocked` or `revision-required`, route only the affected scope back through replan/revision; do not transition the functional-slice acceptance boundary to a terminal Jira workflow state.
