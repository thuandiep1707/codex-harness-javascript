...
Read by task instead of loading the full file:

| Task                               | Required sections                                                                                |
| ---------------------------------- | ------------------------------------------------------------------------------------------------ |
| Identify an item address           | `Address Schemes`; add `GitHub Registries` for `owner/repo/item`                                 |
| Prepare external adoption evidence | `Address Schemes`, source-specific section, and `Registry Dependencies`                          |
| Author or repair a source registry | `Mental Model`, `Root registry.json`, `Include`, `Item Definitions`, and `Registry Dependencies` |
| Build or verify a registry         | Authoring sections plus `Build and Verify`                                                       |

Identity inspection does not authorize adoption. Preserve the exact address and
version or resolved commit in the supply-chain handoff.
