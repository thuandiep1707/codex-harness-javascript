...
This is a lookup reference, not a start-to-finish workflow. Find the required
heading (for example, `rg -n '^### `add`' cli.md`) and read only that command or
preset section plus its immediate prerequisites.

> **IMPORTANT:** Use the project's package runner and lockfile-resolved shadcn
> version. Examples below use generic `npx shadcn@latest` syntax for upstream
> portability; do not switch this repository to `@latest` unless an approved
> upgrade explicitly authorizes it.

> **IMPORTANT:** Only use the flags documented below. Do not invent or guess flags — if a flag isn't listed here, it doesn't exist. The CLI auto-detects the package manager from the project's lockfile; there is no `--package-manager` flag.

## Contents

- Component discovery/update: `add`, `search`, `view`, `docs`, `diff`, `info`.
- Project setup: `init`; preset mutation: `apply`, `Presets`, `Switching Presets`.
- Registry authoring: `build` plus the required sections in `registry.md`.
...
See [Updating Components in SKILL.md](./SKILL.md#update-existing-components-safely) for the full workflow.
...
- **Merge** → `npx shadcn@latest init --preset <code> --force --no-reinstall`, then run `npx shadcn@latest info` to get the list of installed components and use the [safe update workflow](./SKILL.md#update-existing-components-safely) to update them one by one, preserving local changes. Use when the user has customized components.
