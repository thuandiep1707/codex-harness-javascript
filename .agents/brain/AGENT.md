...
7. Return one YAML `analysis-package`. Set `analysis-status: ready` only when `authority.status: ready`; otherwise return `analysis-status: blocked` with authority blockers. Do not create tasks, Jira issues, specialist assignments, or adopt new dependencies.
...
3. Compare requirements with the approved Jira Feature/Task context, durable specialist results, current source changes, and only still-valid validation evidence. A green suite covers only acceptance criteria explicitly evidenced by its report.
