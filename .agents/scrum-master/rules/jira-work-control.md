...
- During `progress-sync`, mutate Jira only from confirmed durable evidence supplied by Main. Test Plan/Testing Logic/Testing UI self-verification stays transient and never authorizes creation of Jira execution units. Never infer specialist completion, validation, cleanup, source state, or acceptance.
