...
- During `progress-sync`, mutate Jira only from confirmed durable evidence supplied by Main. Test Plan/Testing Logic/Testing UI self-verification stays transient and never creates separate Jira work items. For Coding lifecycle, the same Jira work item may move todo/open -> active/in-progress before Coding, active/non-terminal -> completed after confirmed self-verification, or back/remain active with a durable `[REVISION]` when Testing proves a production defect. Resolve actual project transitions first; never infer completion, validation, cleanup, source state, or acceptance.
