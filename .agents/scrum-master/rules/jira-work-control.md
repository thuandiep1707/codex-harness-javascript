...
- During planning/replan, persist the current analysis/authority readiness, context-version, docs-baseline, and relevant-document references needed for a later resume without chat history.
...
- During `progress-sync`, mutate Jira only from confirmed durable evidence supplied by Main. Test Plan/Testing Logic/Testing UI stay transient and never create separate Jira work items. A Coding work item may move todo/open -> active/in-progress before Coding, then -> completed after its per-task Logic gate passes or is not required. If UI is required, preserve its bounded pending UI targets in that Coding `[RESULT]`. The functional-slice UI gate may persist its own compact `[RESULT]`. Any proven production defect writes `[REVISION]` on the affected existing Coding work item(s) and keeps/returns them active. Resolve actual project transitions first; never infer completion, validation, cleanup, source state, or acceptance.
