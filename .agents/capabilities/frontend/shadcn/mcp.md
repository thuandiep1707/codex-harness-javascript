...
install items from registries. Load this reference only when a configured or
available shadcn MCP is explicitly in scope; ordinary CLI work does not need it.

MCP search/view is inspect-only evidence under the repository workflow. An MCP
install action is a mutation and requires the same source, dependency, preview,
and approval gates as the CLI.
...
```bash
npx shadcn mcp        # start the lockfile-resolved MCP server (stdio)
npx shadcn mcp init   # write config for your editor
```
...
> project configuration (aliases, framework, Tailwind version), use the
> lockfile-resolved `npx shadcn info`; there is no MCP equivalent.
