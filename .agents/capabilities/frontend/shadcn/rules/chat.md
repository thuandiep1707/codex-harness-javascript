...
Use this API reference only after an approved design selects these exact chat
artifacts and the adoption gate below is complete. It describes shipped parts
and behavior; it does not choose the product pattern, DDD placement, styling,
state owner, accessibility contract, attachment policy, or realtime behavior.
...
`useMessageScrollerVisibility`, and `useMessageScrollerScrollable`. Some
registry versions provide them through `@shadcn/react`; verify that the approved
CLI result actually installs the exact package and compatible version before
importing it. Reach for the hooks only when composition cannot express the
required behavior, and route their browser/runtime lifecycle through the
repository runtime rule.
